...
interface ProdutosTabProps {
  produtos: Produto[];
  getPromocoesDoProduto: (itemKey: string) => PromocaoResolvida[];
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-[#D8D9C9]">
                {produtos.map(p => {
                  ...
                  const promocoes = getPromocoesDoProduto(itemKey);
                  return (
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {promocoes.length > 0 && (
                          <PromocaoBadge
                            label={
                              promocoes.length === 1
                                ? `Promoção: ${promocoes[0].mercado}`
                                : `Promoção em ${promocoes.length} mercados`
                            }
                            title={promocoes
                              .map(promo => `${promo.mercado}: ${fmtMoeda(promo.preco)} (até ${promo.validadeTexto || '—'})`)
                              .join('\n')}
                          />
                        )}
                      </div>
